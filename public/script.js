...
const SOLO_TOPIC_CATALOG = {
  Math: [
    { min: 1, max: 3, topics: ["Whole Numbers", "Number Operations", "Fractions", "Measurement", "Data & Graphs"] },
    { min: 4, max: 6, topics: ["Whole Numbers", "Fractions", "Decimals", "Percentages", "Geometry", "Measurement", "Data & Graphs"] },
    { min: 7, max: 8, topics: ["Number Operations", "Fractions", "Decimals", "Percentages", "Ratios & Proportions", "Basic Algebra", "Geometry", "Statistics"] },
    { min: 9, max: 9, topics: ["Integers", "Algebra", "Equations", "Coordinate Geometry", "Probability", "Financial Mathematics"] },
    { min: 10, max: 11, topics: ["Algebra", "Linear Equations", "Functions", "Quadratics", "Trigonometry", "Financial Mathematics", "Statistics"] }
  ],
  English: [
    { min: 1, max: 3, topics: ["Reading Comprehension", "Spelling", "Vocabulary", "Grammar", "Sentence Structure", "Creative Writing"] },
    { min: 4, max: 6, topics: ["Reading Comprehension", "Parts of Speech", "Punctuation", "Paragraph Writing", "Poetry", "Short Stories"] },
    { min: 7, max: 9, topics: ["Reading Comprehension", "Grammar", "Essay Writing", "Literary Devices", "Argumentative Writing", "Critical Thinking"] },
    { min: 10, max: 11, topics: ["Literature", "Literary Analysis", "Persuasive Writing", "Research & Information Literacy", "Speaking & Communication"] }
  ],
  Science: [
    { min: 1, max: 3, topics: ["Living Things", "Plants", "Animals", "Human Body", "Weather"] },
    { min: 4, max: 6, topics: ["Ecosystems", "Environment", "Earth Science", "Matter", "Energy", "Space"] },
    { min: 7, max: 9, topics: ["Cells", "Ecosystems", "Climate", "Matter", "Forces & Motion", "Electricity"] },
    { min: 10, max: 11, topics: ["Biology", "Chemistry", "Physics", "Energy", "Electricity", "Environment"] }
  ],
  "Social Studies": [
    { min: 1, max: 6, topics: ["Citizenship", "Jamaican Society", "Culture & Heritage", "Rights & Responsibilities", "Government & Civics"] },
    { min: 7, max: 9, topics: ["Jamaican Government", "Caribbean Society", "Migration", "Economics", "Youth & Society", "Current Issues"] },
    { min: 10, max: 11, topics: ["Politics", "Jamaican Government", "Economics", "Business", "Migration", "Current Issues"] }
  ],
  Geography: [
    { min: 1, max: 6, topics: ["Jamaican Geography", "Map Skills", "Weather", "Landforms", "Oceans & Rivers"] },
    { min: 7, max: 9, topics: ["Caribbean Geography", "World Geography", "Climate", "Natural Resources", "Population", "Migration"] },
    { min: 10, max: 11, topics: ["Physical Geography", "Human Geography", "Environment", "Climate", "Population", "Migration"] }
  ],
  History: [
    { min: 1, max: 6, topics: ["Taíno Jamaica", "Spanish Jamaica", "National Heroes", "Emancipation", "Culture & Heritage"] },
    { min: 7, max: 9, topics: ["British Capture of Jamaica", "Plantation Society", "Enslavement", "Resistance", "Maroons", "Morant Bay Rebellion"] },
    { min: 10, max: 11, topics: ["Jamaica's Road to Independence", "Independence", "Modern Jamaica", "Resistance", "Emancipation", "National Heroes"] }
  ],
  Music: [
    { min: 1, max: 6, topics: ["Jamaican Music", "Mento", "Gospel", "Musical Instruments", "Rhythm"] },
    { min: 7, max: 9, topics: ["Ska", "Rocksteady", "Reggae", "Dancehall", "Jamaican Folk Music", "Music History"] },
    { min: 10, max: 11, topics: ["Reggae", "Dancehall", "Music Theory", "Keyboard/Piano", "Music History", "Melody"] }
  ]
};

function soloTopicsForGrade(subject, grade) {
  const gradeNumber = Number(grade);
  const groups = SOLO_TOPIC_CATALOG[subject] || [];
  return groups.find((group) => gradeNumber >= group.min && gradeNumber <= group.max)?.topics || [subject];
}
...
function renderSolo(step) {
  ...
  if (step === 1) {
    ...
    const topics = soloTopicsForGrade(soloState.subject, soloState.grade);
    if (!soloState.topic || !topics.includes(soloState.topic)) soloState.topic = topics[0];
    content = `
      <p class="section-heading">Choose Topic</p>
      <div class="debate-category-list solo-topic-list">
        ${topics.map((topic) => `<button class="debate-category-btn${soloState.topic === topic ? " selected" : ""}" type="button" data-topic="${topic}">${topic}</button>`).join("")}
      </div>
      <button class="btn-primary" id="soloNext">Continue</button>
      <p class="status-note center" id="soloStatus"></p>`;
  } else if (step === 4) {
    content = `
      <p class="section-heading">${soloState.topic}</p>
      <div class="solo-progression">
        <span class="active">LEARN</span><span>PRACTICE</span><span>CHALLENGE</span><span>QUIZ</span>
      </div>
      <div class="solo-study-card">
        <p class="status-note">Study this topic, then test your understanding with focused questions.</p>
        <p class="solo-study-title">${soloState.subject} - Grade ${soloState.grade}</p>
        <p class="solo-study-copy">Your quiz will use questions tagged specifically for ${soloState.topic}. If no exact questions are available yet, the session will tell you instead of mixing unrelated material.</p>
      </div>
      <button class="btn-primary btn-gold" id="soloStart">Start Practice</button>
      <p class="status-note center" id="soloStatus"></p>`;
  }
  ...
  if (step === 1) {
    ...
    document.querySelectorAll("[data-topic]").forEach((button) => {
      button.onclick = () => {
        document.querySelectorAll("[data-topic]").forEach((item) => item.classList.remove("selected"));
        button.classList.add("selected");
        soloState.topic = button.dataset.topic;
      };
    });
    document.getElementById("soloNext").onclick = () => {
      if (!soloState.topic || !topics.includes(soloState.topic)) {
        document.getElementById("soloStatus").textContent = "Please select a topic.";
        return;
      }
      renderSolo(4);
    };
    ...
    document.getElementById("soloStart").onclick = async () => {
      ...
      try {
        const session = await api("/api/session/create", "POST", {
          ...
          category: soloState.topic
        });
        startQuiz({
          sessionId: session.sessionId,
          modeLabel: "Solo Study",
          topic: `${soloState.grade} | ${soloState.subject} | ${soloState.topic}`
        });
      } catch (error) {
        status.textContent = error.message;
      }
