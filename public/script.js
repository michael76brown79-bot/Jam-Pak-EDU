...
function renderDebate() {
  ...
  if (debateState.mode === "create") {
    content = `
      ...
      <p class="status-note center">Choose a fiery question, then invite others to defend their side.</p>
      <div class="debate-category-list" role="tablist" aria-label="Debate categories">
        ${categoryNames.map((category) =>
          `<button class="debate-category-btn${debateState.category === category ? " selected" : ""}" type="button" data-category="${category}">${category}</button>`
        ).join("")}
      </div>
      <p class="section-heading">${debateState.category} questions</p>
      <div class="debate-topics">
        ${topics.map((topic, index) =>
          `<button class="debate-topic-btn${selectedTopicLabel === topic ? " selected" : ""}" data-index="${index}">${topic}</button>`
        ).join("")}
      </div>
      <label class="field-label" for="customTopic">Create your own question</label>
      <textarea id="customTopic" rows="3" placeholder="Write a question your group will want to argue about...">${customTopicValue}</textarea>
