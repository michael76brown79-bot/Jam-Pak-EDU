...
function avatarCardsMarkup(selected) {
  return AVATAR_CHOICES.map((avatarId) =>
    `<button class="avatar-card${selected === avatarId ? " selected" : ""}" type="button" data-avatar="${avatarId}" aria-label="Choose avatar">
  ...
  const historyRowsHTML = recentHistory.length
    ? recentHistory.map((h) => `
        <div class="sd-history-row">
          <span class="sd-history-subject">${h.subject || "Quiz"} Â· Grade ${h.grade || "?"}</span>
    ...
    <div class="page">
      ...
      <div class="tp-container">
        ...
        <div class="sd-hero">
          ...
          <div class="sd-hero-info">
            ...
            <div class="sd-hero-details">
              <span>ðŸ« ${schoolName}</span>
              <span>ðŸ‘©â€ðŸ« ${teacherName}</span>
              <span>ðŸ›ï¸ ${className}</span>
            </div>
          </div>
          <div class="sd-hero-btns">
            <button class="btn-back" id="studentHomeBtn">ðŸ  Home</button>
        ...
        <div class="tp-stats-row" style="grid-template-columns:repeat(3,1fr)">
          ...
          <div class="tp-stat">
            <div class="tp-stat-num">${avgScore}%</div>
            <div class="tp-stat-label">Avg Score</div>
          </div>
        </div>

        <div class="sd-quick-grid">
          <div class="sd-action-card" id="sdSoloBtn">
            <div class="sd-action-icon">ðŸ“š</div>
          ...
          <div class="sd-action-card sd-action-gold" id="sdJoinBtn">
            <div class="sd-action-icon">ðŸŽ®</div>
...
async function renderTeacherPortal() {
  ...
  const classCardsHTML = classes.map((cls) => `
    <div class="tp-class-card">
      <div class="tp-class-icon">ðŸ«</div>
      ...
      <div class="tp-class-actions">
        <button class="tp-class-btn tp-view-btn" data-classid="${cls.classId}">ðŸ‘ View</button>
  ...
  const quizCardsHTML = quizzes.map((quiz) => `
    <div class="tp-quiz-card">
      ...
      <button class="btn-primary btn-gold tp-launch-btn" data-quizid="${quiz.quizId}">ðŸš€ Start Session</button>
  ...
  const reviewsHTML = pendingReviews.map((rev) => `
    <div class="tp-review-item" id="tp-review-${rev.reviewId || rev.answerId}">
      ...
      <div class="tp-review-btns">
        <button class="btn-primary tp-review-correct-btn" data-id="${rev.reviewId || rev.answerId}">âœ… Correct</button>
        <button class="btn-back tp-review-incorrect-btn" data-id="${rev.reviewId || rev.answerId}">âŒ Incorrect</button>
      </div>
    </div>`).join("");

  const reviewTabLabel = pendingReviews.length
    ? `<button class="tp-tab tp-tab-alert" data-tab="reviews">âš ï¸ Reviews (${pendingReviews.length})</button>`
    : "";
      ...
      <div class="tp-container">
        ...
        <div class="tp-header">
          <div class="tp-header-left">
            <div class="tp-icon-wrap">ðŸ‘©â€ðŸ«</div>
            <div>
              <div class="tp-welcome-line">Teacher Portal</div>
              <div class="tp-name-line">${teacherName}</div>
              ${schoolName ? `<div class="tp-school-line">ðŸ« ${schoolName}</div>` : ""}
            </div>
          </div>
          <div class="tp-header-btns">
            <button class="btn-back" id="teacherPortalBackBtn">ðŸ  Home</button>
        ...
        <div class="tp-tabs">
          <button class="tp-tab active" data-tab="classes">ðŸ“š Classes</button>
          <button class="tp-tab" data-tab="quizzes">ðŸ“ Quizzes</button>
          <button class="tp-tab" data-tab="create">âž• Create Quiz</button>
          <button class="tp-tab" data-tab="questions">ðŸ”§ Questions</button>
          ${reviewTabLabel}
        </div>
        ...
        <div class="tp-section hidden" id="tp-tab-quizzes">
          ...
          <div class="tp-empty">
            <div class="tp-empty-icon">ðŸ“</div>
  ...
  document.getElementById("tpNewClassCard").onclick = () => {
    ...
    document.getElementById("tpCreateClassBtn").onclick = async () => {
      ...
      try {
        await api("/api/teacher/classes/create", "POST", { className }, teacherState.accessToken);
  ...
  document.querySelectorAll(".tp-view-btn").forEach((btn) => {
    btn.onclick = async () => {
      ...
      const panel = document.getElementById("tpInlinePanel");
      panel.innerHTML = `<div class="tp-inline-form"><p class="status-note">Loading studentsâ€¦</p></div>`;
      try {
  ...
  document.querySelectorAll(".tp-launch-btn").forEach((btn) => {
    btn.onclick = async () => {
      ...
      try {
        ...
        statusEl.textContent = `ðŸŽ‰ Room Code: ${result.roomCode}`;
      } catch (error) {
...
function renderHost(step) {
  ...
  if (step === 1) {
    ...
    content = `
      ...
      <div class="lobby-actions">
        ...
        <button class="btn-back btn-inline" id="shareLobbyBtn">Share Code</button>
  ...
  if (step === 1) {
    ...
    document.getElementById("shareLobbyBtn").onclick = async () => {
      ...
      try {
        if (navigator.share) {
          await navigator.share({ title: "Jam-Pak EDU", text: shareText });
          lobbyInfo.textContent = "Room code shared.";
        } else if (navigator.clipboard && navigator.clipboard.writeText) {
          await navigator.clipboard.writeText(hostState.roomCode);
          lobbyInfo.textContent = "Room code copied. Share it with players.";
        } else {
          lobbyInfo.textContent = `Room code: ${hostState.roomCode}`;
        }
      } catch (_error) {
        lobbyInfo.textContent = "Could not share right now. Please copy and send the room code manually.";
      }
    };
      ...
      <div class="inner-page inner-page-wide">
        ...
        <div class="panel">
          ...
          <input type="text" id="joinCode" class="code-input"
                 placeholder="JAM01234" maxlength="8" autocomplete="off"/>
...
function renderDebate() {
  const topics = [
    "Should homework be abolished in Jamaican schools?",
    "Is social media beneficial to Jamaican students?",
    "Should CSEC exams include more project work?",
    "Should Jamaican schools teach more local history?",
    "Is tourism Jamaica's most important future industry?"
  ];
  if (!debateState.grade) debateState.grade = "7";
  if (!isSubjectAllowedForGrade(debateState.grade, debateState.subject)) {
    debateState.subject = subjectsForGrade(debateState.grade)[0]?.value || "";
  }
  ...
  if (debateState.mode === "create") {
    content = `
      <p class="section-heading">Create a Debate Room</p>
      <p class="status-note center">Pick a topic, choose your grade/category, and invite others to join your debate.</p>
      <div class="debate-topics">
        ${topics.map((topic, index) =>
          `<button class="debate-topic-btn${selectedTopicLabel === topic ? " selected" : ""}" data-index="${index}">${topic}</button>`
        ).join("")}
      </div>
      <label class="field-label" for="customTopic">Or create your own debate prompt</label>
      <input type="text" id="customTopic" placeholder="Type your debate question here" value="${customTopicValue}"/>
      <p class="section-heading">Grade</p>
      <div class="grade-grid grade-grid-3">
        ${GRADE_LEVELS.map((grade) => gradeCardMarkup(debateState.grade, grade)).join("")}
      </div>
      <p class="section-heading">Category</p>
      <div class="subject-grid">${subjectCardsMarkup(debateState.subject, debateState.grade)}</div>
      <button class="btn-primary btn-gold" id="debateCreateRoomBtn">Create Debate Room</button>
  ...
  if (debateState.mode === "create") {
    ...
    bindGradeSelection((grade) => {
      debateState.grade = grade;
      if (!isSubjectAllowedForGrade(debateState.grade, debateState.subject)) {
        debateState.subject = subjectsForGrade(debateState.grade)[0]?.value || "";
      }
      renderDebate();
    });
    bindSubjectSelection((subject) => { debateState.subject = subject; });
    ...
    document.getElementById("debateCreateRoomBtn").onclick = () => {
      ...
      if (!isSubjectAllowedForGrade(debateState.grade, debateState.subject)) {
        status.textContent = "Please select a valid category for that grade.";
        return;
      }
...
async function loadNextQuestion() {
  ...
  try {
    ...
    if (data.waiting) {
      ...
      const reason = data.waitReason === "players"
