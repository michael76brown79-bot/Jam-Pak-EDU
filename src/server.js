...
if (!Array.isArray(accessControl.assignments)) {
  accessControl.assignments = [];
  saveAccessControl(accessControl);
}
accessControl.students.forEach((student) => {
  if (!Array.isArray(student.memberships)) student.memberships = normalizeStudentMemberships(student);
});
accessControl.classes.forEach((teacherClass) => {
  if (!teacherClass.classCode) teacherClass.classCode = createClassCode(teacherClass.gradeBand);
});
saveAccessControl(accessControl);
...
function listTeacherClasses(teacherId) {
  return accessControl.classes.filter((cls) => cls.teacherId === teacherId).map((cls) => ({
    ...cls,
...
function listTeacherAssignments(teacherId) {
  return accessControl.assignments.filter((assignment) => assignment.teacherId === teacherId);
}

function classCodeExists(classCode) {
  return accessControl.classes.some((item) => item.classCode === classCode);
}

function createClassCode(gradeBand) {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    const grade = String(gradeBand || "X").replace(/[^0-9A-Z]/gi, "").slice(0, 3).toUpperCase() || "X";
    const code = `JPK-${grade}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
    if (!classCodeExists(code)) return code;
  }
  throw new Error("Could not create a unique class code");
}

function normalizeStudentMemberships(student) {
  if (Array.isArray(student.memberships)) return student.memberships;
  if (student.teacherId || student.classId) {
    return [{
      teacherId: student.teacherId || null,
      classId: student.classId || null,
      joinedAt: student.createdAt || Date.now()
    }];
  }
  return [];
}
...
function publicStudent(student) {
  return {
    ...
    memberships: normalizeStudentMemberships(student)
  };
}
...
app.post("/api/auth/student/register", (req, res) => {
  ...
  const student = {
    ...
    memberships: linkedClass ? [{ teacherId: teacher.teacherId, classId: linkedClass.classId, joinedAt: Date.now() }] : [],
    createdAt: Date.now()
  };
...
app.post("/api/teacher/classes/create", (req, res) => {
  ...
  if (!auth) return;
  const { className, gradeBand, subject } = req.body || {};
  ...
  const teacherClass = {
    ...
    gradeBand: String(gradeBand || "").trim() || null,
    subject: String(subject || "").trim() || null,
    classCode: createClassCode(gradeBand),
    students: [],
    createdAt: Date.now()
  };
...
app.post("/api/student/classes/join", (req, res) => {
  ...
  const teacherClass = accessControl.classes.find((item) => item.classCode === classCode);
  if (!teacherClass) return res.status(404).json({ error: "Class code not found." });

  const student = accessControl.students.find((item) => item.studentId === auth.studentId);
  if (!student) return res.status(404).json({ error: "Student account not found." });
  student.memberships = normalizeStudentMemberships(student);
  if (!student.memberships.some((item) => item.classId === teacherClass.classId)) {
    student.memberships.push({ teacherId: teacherClass.teacherId, classId: teacherClass.classId, joinedAt: Date.now() });
  }
  teacherClass.students = Array.isArray(teacherClass.students) ? teacherClass.students : [];
  if (!teacherClass.students.some((item) => item.studentId === student.studentId)) {
    teacherClass.students.push({ studentId: student.studentId, name: student.name, avatarId: student.avatarId || "island-ace" });
  }
  saveAccessControl(accessControl);
  return res.json({ ok: true, student: publicStudent(student), class: {
    classId: teacherClass.classId,
    className: teacherClass.className,
    classCode: teacherClass.classCode,
    teacherId: teacherClass.teacherId,
    gradeBand: teacherClass.gradeBand,
...
app.get("/api/student/classes", (req, res) => {
  ...
  const student = accessControl.students.find((item) => item.studentId === auth.studentId);
  if (!student) return res.status(404).json({ error: "Student account not found." });
  const memberships = normalizeStudentMemberships(student);
  const classes = memberships.map((membership) => {
    const teacherClass = accessControl.classes.find((item) => item.classId === membership.classId);
    const teacher = accessControl.teachers.find((item) => item.teacherId === membership.teacherId);
    if (!teacherClass) return null;
    return { classId: teacherClass.classId, className: teacherClass.className, classCode: teacherClass.classCode, gradeBand: teacherClass.gradeBand, subject: teacherClass.subject, teacherId: teacherClass.teacherId, teacherName: teacher?.name || "Teacher" };
  }).filter(Boolean);
  return res.json({ classes });
});
...
app.post("/api/teacher/assignments/create", (req, res) => {
  const auth = requireRole(req, res, ["teacher"]);
  if (!auth) return;
  const { classId, title, instructions, activityType = "Quiz", dueDate = "", points = 100 } = req.body || {};
  if (!classId || !title) return res.status(400).json({ error: "classId and title are required" });
  const teacherClass = accessControl.classes.find((item) => item.classId === classId && item.teacherId === auth.teacherId);
  if (!teacherClass) return res.status(404).json({ error: "Class not found" });
  const assignment = {
    assignmentId: `as_${Date.now()}_${Math.floor(Math.random() * 100000)}`,
    teacherId: auth.teacherId,
    classId,
    title: String(title).trim(),
    instructions: String(instructions || "").trim(),
    activityType: String(activityType).trim() || "Quiz",
    dueDate: String(dueDate || "").trim(),
    points: Math.max(1, Math.min(1000, Number(points) || 100)),
    submissions: [],
    createdAt: Date.now()
  };
  accessControl.assignments.push(assignment);
  saveAccessControl(accessControl);
...
app.get("/api/teacher/assignments", (req, res) => {
  const auth = requireRole(req, res, ["teacher"]);
  if (!auth) return;
  return res.json({ assignments: listTeacherAssignments(auth.teacherId) });
});

app.get("/api/teacher/classes/:classId/progress", (req, res) => {
  const auth = requireRole(req, res, ["teacher"]);
  if (!auth) return;
  const teacherClass = accessControl.classes.find((item) => item.classId === req.params.classId && item.teacherId === auth.teacherId);
  if (!teacherClass) return res.status(404).json({ error: "Class not found" });
  const assignments = listTeacherAssignments(auth.teacherId).filter((item) => item.classId === teacherClass.classId);
  const students = (teacherClass.students || []).map((student) => {
    const scores = assignments.flatMap((assignment) => (assignment.submissions || [])
      .filter((submission) => submission.studentId === student.studentId && submission.grade !== null && submission.grade !== undefined)
      .map((submission) => submission.grade));
    return { ...student, assignmentAverage: scores.length ? Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length) : null, assignmentsCompleted: scores.length };
  });
  const allScores = students.filter((student) => student.assignmentAverage !== null).map((student) => student.assignmentAverage);
  return res.json({ class: teacherClass, assignments, students, classAverage: allScores.length ? Math.round(allScores.reduce((sum, score) => sum + score, 0) / allScores.length) : null });
});
...
app.get("/api/student/assignments", (req, res) => {
  ...
  const student = accessControl.students.find((item) => item.studentId === auth.studentId);
  if (!student) return res.status(404).json({ error: "Student account not found." });
  const classIds = new Set(normalizeStudentMemberships(student).map((membership) => membership.classId));
  const assignments = accessControl.assignments.filter((assignment) => classIds.has(assignment.classId)).map((assignment) => {
    const submission = (assignment.submissions || []).find((item) => item.studentId === student.studentId) || null;
    const teacherClass = accessControl.classes.find((item) => item.classId === assignment.classId);
    const teacher = accessControl.teachers.find((item) => item.teacherId === assignment.teacherId);
    return { ...assignment, submissions: undefined, submission, className: teacherClass?.className || "Class", teacherName: teacher?.name || "Teacher" };
  });
...
app.post("/api/student/assignments/:assignmentId/submit", (req, res) => {
  ...
  if (!auth) return;
  const student = accessControl.students.find((item) => item.studentId === auth.studentId);
  const assignment = accessControl.assignments.find((item) => item.assignmentId === req.params.assignmentId);
  if (!student || !assignment) return res.status(404).json({ error: "Assignment not found" });
  const member = normalizeStudentMemberships(student).some((membership) => membership.classId === assignment.classId);
  if (!member) return res.status(403).json({ error: "You are not enrolled in this class." });
  const existing = assignment.submissions.find((item) => item.studentId === student.studentId);
  const submission = { studentId: student.studentId, response: String(req.body?.response || "").trim(), submittedAt: Date.now(), grade: existing?.grade ?? null, feedback: existing?.feedback || "" };
...
app.post("/api/teacher/assignments/:assignmentId/grade", (req, res) => {
  const auth = requireRole(req, res, ["teacher"]);
  if (!auth) return;
  const assignment = accessControl.assignments.find((item) => item.assignmentId === req.params.assignmentId && item.teacherId === auth.teacherId);
  if (!assignment) return res.status(404).json({ error: "Assignment not found" });
  const studentId = String(req.body?.studentId || "").trim();
  const grade = Number(req.body?.grade);
  if (!studentId || !Number.isFinite(grade)) return res.status(400).json({ error: "studentId and numeric grade are required" });
  const submission = assignment.submissions.find((item) => item.studentId === studentId);
  if (!submission) return res.status(404).json({ error: "Submission not found" });
  submission.grade = Math.max(0, Math.min(100, Math.round(grade)));
...
app.post("/api/teacher/quizzes/:quizId/start", (req, res) => {
  ...
  session.hostPlayerId = hostPlayer.playerId;
...
app.post("/api/session/create", (req, res) => {
  const { mode, gradeBand, subject, category, hostName, avatarId } = req.body || {};
  if (!mode || !gradeBand || !subject) {
    return res.status(400).json({ error: "mode, gradeBand, subject are required" });
  }

  const session = createSession({ mode, gradeBand, subject: category || subject });
  session.filters.subject = subject;
  session.filters.category = category || subject;
  session.players     = [];
  let hostPlayer = null;
  if (hostName) {
  ...
  session.hostPlayerId = hostPlayer ? hostPlayer.playerId : null;
...
app.post("/api/session/join", (req, res) => {
  ...
  return res.json({
    ...
    started: Boolean(session.started),
    waitingForHost: !session.started,
    leaderboard: leaderboardFor(session)
  });
});
...
app.post("/api/session/:sessionId/start", (req, res) => {
  ...
  if (!playerId || playerId !== session.hostPlayerId) {
...
app.get("/api/session/:sessionId/next-question", (req, res) => {
  ...
  if (multiplayer) {
    if (!session.started) {
      return res.json({ done: false, waiting: true, waitingForHost: true, waitReason: "host" });
    }
    ...
    const shouldAdvance = session.waitForAllPlayers ? progress.everyoneAnswered : hasAnswered;
